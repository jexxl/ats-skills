...
UNRESOLVED_PLACEHOLDER_RE = re.compile(
    r"\{[^{}\n]*(?:算法|模型|输入|输出|文件|字段|参数|默认值|取值|说明|当前日期|粘贴|报告|数据|配置|结论)[^{}\n]*\}"
)
...
def main() -> None:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="algorithm-docs-"))
    try:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build algorithm packaging Word documents from Markdown.")
...
def build_basic_info_table(metadata: dict, inputs: list[dict], outputs: list[dict]) -> list[str]:
    ...
    rows.extend(
        [
            ...
            ["责任单位", meta_value(metadata, "责任单位", "同济大学")],
        ]
    )
